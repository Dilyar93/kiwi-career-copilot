...
The Career Library accepts PDF, DOCX, Markdown, and text sources. Each file is limited to 2 MB and 100,000 extracted characters; PDFs are also limited to 50 pages. The original and extracted text are saved locally before the extracted text is sent to the configured model for classification and source-bound Claim extraction. Claims are retrieval indexes, not a checklist for the user. Only real cross-source conflicts or failed model understanding appear for review. During job analysis, the Agent decides whether to query Claims, search chunks, open a specific original, or stop. Generic search omits highly sensitive raw text; the Agent can deliberately select a known sensitive source when the job requires it. Image-only PDFs require OCR first and legacy `.doc` files are unsupported.
