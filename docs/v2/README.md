...
V2 首先是一个能被真实使用、愿意持续使用的产品。Stage 2 结束时已冻结一份实习作品集快照；完整 evaluation、最终开源/展示更新和商店发布仍是后续结果，不能反过来主导当前产品设计。

## 一句话定义

一个以 SEEK 为当前入口的个人求职 Agent：它从用户的多份 CV、项目、工作权利及其他职业来源建立可追溯知识库，帮助理解岗位、判断是否申请、准备多份岗位材料、记录申请，并逐步形成职业能力改进计划。当前实现需要本机 Agent 服务，并明确使用云端模型处理披露过的内容；未来交付方式尚未确定。
...
阅读顺序用于先建立当前事实；以下优先级只用于文档之间发生真实冲突时，不要求 AI 每次完整读取所有文件。
