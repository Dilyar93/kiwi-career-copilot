...
| 展示 | tool trace、架构图、演示脚本和结果叙事被列为主要交付 | 默认 UI 先服务用户任务；trace 属于详情，展示包装最后做 | 面试叙事不能主导产品信息架构 |
| Trade Me | V1/V2 曾计划双站点 | V2 SEEK-only，Trade Me 退役 | 上线后确认 Trade Me 不可靠；不继续投入修复 |
| 网站扩展 | 提前设计多个来源和注册机制 | 列表增强与详情读取分开；阶段 3 增加“任意公司官网当前详情页手动读取” | 满足真实资料补全需求，但不做通用列表识别、注入或提前建设插件系统 |
...
| Job enrichment | 早期两周范围外 | 保留为 Job 工作台阶段的重要产品能力 | SEEK 信息不足和官网来源是用户真实痛点，但应在核心 UX 稳定后做 |
| RAG / Retrieval | 原企划包含向量检索，之后延后并先做结构化 evidence/tag 与逐行搜索 | Stage 2 建立 Claim → chunk → parent/original 的分层检索；FTS5 先行并为混合语义检索留出明确增量 | 多份 CV、Visa 和项目资料带来了真实的跨来源召回需求；仍不引入 GraphRAG 或外部向量基础设施 |
