...
状态：阶段 1 设计基线已实现并通过作者真实 SEEK 走查；阶段 2 已把 Profile-first 入口替换为 Source-only Career Library，正在真实资料走查。实现变化若影响用户旅程或界面层级，应同步更新本文。
...
- 日常判断：切换 SEEK 岗位 → Kiwi 自动同步 → 查看实时检查 → 必要时回答 Agent 追问 → 查看是否值得申请、硬性风险与下一步 → 标记准备/待考虑/跳过。
- 准备申请：从已完成分析确认计划 → 生成并编辑岗位材料 → 下载 → 用户自行检查和提交。
...
├─ Needs your attention（按需出现）：只处理跨来源真实冲突或高风险问题
...
5. 若结论为 `needs_clarification`，在申请动作前显示问题与回答框；全部回答后继续同一岗位分析。
6. 用户操作：准备、稍后、已申请、跳过。待澄清时隐藏这些动作，避免把临时判断当最终结论。
...
- Agent trace 默认折叠。
- `needs_clarification` 不是只读清单；用户可以回答，继续运行后得到新的结构化结论。
